...
APEX_cpu_init(const char* filename,const char* function_code,const char* function_cycles)
{
    ...
    memset(cpu->physicalRegisters, 0, sizeof(Register) * 48);// MARK: - 40 -> 48
    memset(cpu->architectureRegisters, 0, sizeof(int) * 48);
    memset(cpu->allocationList, 0, sizeof(int) * 48);
    memset(cpu->renameTable, 0, sizeof(int) * 48);
    memset(cpu->backendRAT, 0, sizeof(int) * 48);
    //MARK: - jal rd set
    memset(JAL_rd, -1, sizeof(int) * 48);
    memset(BTB, -1, sizeof(int) * 48);
    ...
    memset(cpu->issueQueue, 0, sizeof(APEX_Instruction) * 24);
    ...
    cpu->freePhysicalRegister=createQueue(48);  //Capacity is equal to maximum number of physical registers;// MARK: - 40 -> 48
    cpu->reorderBuffer=createROBQueue(200); //ROB capacity // MARK: - 64 -> 100
...
static void printStageWithURF(CPU_Stage* stage){
    if(isInstructionMOVC(stage->opcode)){
        ...
        printf("(I%d) %s,R%d,#%d [%s,U%d,#%d]\n",stage->index,stage->opcode,stage->arch_rs1,stage->imm,stage->opcode,stage->rs1,stage->imm);
    }else if(isInstructionJAL(stage->opcode)){
        printf("(I%d) %s,R%d,R%d,#%d [%s,U%d,U%d,#%d]\n",stage->index,stage->opcode,stage->arch_rd,stage->arch_rs1,stage->imm,stage->opcode,stage->rd,stage->rs1,stage->imm);
    }
    //MARK: - CMP
    else if(isInstructionCMP(stage->opcode)){
        printf("(I%d) %s,R%d,R%d [%s,U%d,U%d]\n",
               stage->index,stage->opcode,stage->arch_rs1,stage->arch_rs2,stage->opcode,stage->rs1,stage->rs2);
    }
    //MARK: - LDR
    else if(isInstructionLDR(stage->opcode)){
        printf("(I%d) %s,R%d,R%d,R%d [%s,U%d,U%d,U%d]\n",
               stage->index,stage->opcode,stage->arch_rd,stage->arch_rs1,stage->rs2,stage->opcode,stage->rd,stage->rs1,stage->rs2);
    }
    //MARK: STR
    else if(isInstructionSTR(stage->opcode)){
        printf("(I%d) %s,R%d,R%d,R%d [%s,U%d,U%d,U%d]\n",
               stage->index,stage->opcode,stage->arch_rs1,stage->arch_rs2,stage->arch_rs3,stage->opcode,stage->rs1,stage->rs2,stage->rs3);
    }
...
static void print_instruction(CPU_Stage* stage) {
    if(isInstructionMOVC(stage->opcode)){
        ...
        printf("(I%d) %s R%d,R%d,#%d",stage->index, stage->opcode, stage->rs1, stage->rs2, stage->imm);
    } else if(isInstructionHalt(stage->opcode)){
        ...
        printf("(I%d) %s R%d,#%d",stage->index, stage->opcode, stage->rs1, stage->imm);
    }else if(isInstructionJAL(stage->opcode)){
        printf("(I%d) %s R%d,R%d,#%d",stage->index, stage->opcode, stage->rd, stage->rs1, stage->imm);
    }
    //MARK: - CMP
    else if(isInstructionCMP(stage->opcode)) {
        printf("(I%d) %s R%d,R%d",stage->index,stage->opcode,stage->rs1,stage->rs2);
    }
    //MARK: - LDR
    else if(isInstructionLDR(stage->opcode)){
        printf("(I%d) %s R%d,R%d,R%d",stage->index, stage->opcode, stage->rd, stage->rs1, stage->rs2);
    ...
    //MARK: STR
        ...
        printf("(I%d) %s R%d,R%d,R%d",stage->index, stage->opcode, stage->rs1, stage->rs2, stage->rs3);
    }
}
...
int decode(APEX_CPU* cpu) {
    ...
    if (!stage->busy && !stage->stalled) {
        ...
        else if(isInstructionMUL(stage->opcode))
        {
            ...
            stage->loadStoreCounter = 2; // MARK: - m1 m2
...
//            //MARK: - addl rd update
//            int rd_buffer=getPhysicalFromArchitecture(cpu,stage->rd);
//            if(rd_buffer== -1){
//                int rg=getNextFreePhysicalRegister(cpu);
//                addRenameTableEntry(cpu,stage->arch_rd,rg);
//                stage->rd=rg;
//            }else{
//                stage->rd = rd_buffer;
//            }
        ...
        //MARK: - LDR
            ...
            stage->loadStoreCounter = 2;//MARK: - LOAD STORE counter = 3
            stage->is_mem_address_valid=0;
        }
        //MARK: STR
        else if(isInstructionSTR(stage->opcode)){
            ...
            //getting the value of physical register which is mapped to rs3 architecture register via rename table.
            stage->rs3=getPhysicalFromArchitecture(cpu,stage->rs3);
            if(stage->rs3== -1){
                //Check if no physical registers are free;
                int rg=getNextFreePhysicalRegister(cpu);
                addRenameTableEntry(cpu,stage->arch_rs3,rg);
                stage->rs3=rg;
            }
            stage->rs3_value=cpu->physicalRegisters[stage->rs3].value;
...
int dispatch(APEX_CPU* cpu){
    ...
    if (!stage->busy && !stage->stalled) {
        ...
        else if((isInstructionLoad(stage->opcode) || isInstructionLDR(stage->opcode) || isInstructionStore(stage->opcode) || isInstructionSTR(stage->opcode))){//MARK: - STR
...
//        printf("Inside decode ls Counter %d\n",stage->loadStoreCounter);
                ...
                cpu->stage[DISPATCH].loadStoreCounter=2;
        ...
        //MARK: - MUL
            ...
            if(!(isIssueQueueFull(cpu) || isROBFull(cpu->reorderBuffer))){
                pushToIssueQueue(cpu,cpu->stage[DISPATCH]);
                pushToROB(cpu->reorderBuffer,cpu->stage[DISPATCH]);
                //MARK: - mulcounter
                cpu->stage[DISPATCH].mulCounter=3;
            }else{
                cpu->stage[DISPATCH].stalled=1;
                //Stall this Decode stage.
            }
        ...
        //MARK: - BTB
        else if(isInstructionBNZ(stage->opcode) || isInstructionBZ(stage->opcode)){
            if(!(isIssueQueueFull(cpu) || isROBFull(cpu->reorderBuffer))){
                if (ENABLE_DEBUG_MESSAGES) {
                    printf("Details of BTB State --\n");
                    printStageWithURF(stage);
                    printf("pc: %d\n",stage->pc+stage->imm);
                    printf("--\n");
                }
...
//            }
            ...
            if(cpu->physicalRegisters[stage->rd].value == 0){ // TIPS: - only CMP, SUB, SUBL can set zero flag
                cpu->z_flag=1;
            }else{
                cpu->z_flag=0;
            }
        ...
        //MARK: - LDR
            ...
            for(int i=0;i<20;i++){
                if(cpu->loadStoreQueue->array[i].index == stage->index){
                    cpu->loadStoreQueue->array[i].is_mem_address_valid=1;
                    cpu->loadStoreQueue->array[i].mem_address=stage->rs1_value+stage->rs2_value;
                    break;
                }
            }
        }
        //MARK: - STR
            ...
            for(int i=0;i<20;i++){
                if(cpu->loadStoreQueue->array[i].index == stage->index){
                    cpu->loadStoreQueue->array[i].is_mem_address_valid=1;
                    cpu->loadStoreQueue->array[i].mem_address=stage->rs2_value+stage->rs3_value;
                    break;
                }
            }
        }
...
//            }
        ...
        } else if(isInstructionSUBL(stage->opcode)){ // MARK: - only CMP, SUB, SUBL can set zero flag
            stage->rs1_value=cpu->physicalRegisters[stage->rs1].value;
...
//                strcpy(cpu->stage[DRF].opcode,"");
                ...
                if(cpu->stage[DISPATCH].index > stage->index){
                    strcpy(cpu->stage[DISPATCH].opcode,"");
                    //MARK: -
                    if(cpu->physicalRegisters[cpu->stage[DISPATCH].rd].isValid==0){
                        for(int i=0;i<48;i++){
                            if(cpu->renameTable[i]==cpu->stage[DISPATCH].rd){
                                cpu->renameTable[i] = cpu->backendRAT[i];
                                break;
                            }
                        }
                        makeRegisterValid(cpu,cpu->stage[DISPATCH].rd);
                ...
                for(int i=0;i<24;i++){
                    if(strcmp(cpu->issueQueue[i].opcode,"") !=0){
                        ...
                        if(currentIndex < stage->index && currentIndex > instruction_counter){
                            ...
                            if(cpu->issueQueue[i].rd >-1 && cpu->issueQueue[i].rd < 48){
...
//                                printf("Free the destination due branch taken");
            ...
            if(JAL_rd[stage->rs1]!=-1){   // MARK: - judge if rd is used in jal function call
                stage->rs1_value = JAL_rd[stage->rs1];

                cpu->physicalRegisters[stage->rs1].value=JAL_rd[stage->rs1]; // MARK: -write jal rd
                makeRegisterValid(cpu,stage->rs1);
...
//            }
        ...
        // MARK: - add jal in exe
            ...
            cpu->physicalRegisters[stage->rd].value=stage->pc + 4;  // MARK: - update rd with next ins pc
            JAL_rd[stage->rd] =  stage->pc + 4; // MARK: - get jal_rd in stack
            makeRegisterValid(cpu,stage->rd);
...
//        }
        if(isInstructionMUL(stage->opcode)){
            //MARK: - mulcounter
            cpu->stage[MUL_FU].mulCounter = cpu->stage[MUL_FU].mulCounter-1;
            stage->stalled=1;

            if (ENABLE_DEBUG_MESSAGES) {
                printf("Details of MUL FU - \n");
                printf(" cycle(%d) - ", 3-(stage->mulCounter));
                //MARK: - print mul stage problem
                printStageWithURF(stage);
                printf("--\n");
            }
        }
        ...
        //MARK: - mulcounter;
        stage->mulCounter = stage->mulCounter - 1;
...
//      cpu->stage[MEM]=stage;
            ...
            if (ENABLE_DEBUG_MESSAGES) {
                ...
                printf(" cycle(%d) - ", 2-(stage->loadStoreCounter));
                printStageWithURF(stage);
                printf("--\n");
            }
        }
...
//            printf("LOAD STORE count in cycle 1 -> %d\n",stage->loadStoreCounter);
        ...
        if(isInstructionLoad(stage->opcode)){
            ...
            if(stage->loadStoreCounter == 0){
                // MARK: - give this 0 to target commit store in ROB
                for(int i=0;i<200;i++){
                    if(cpu->reorderBuffer->array[i].index == stage->index){
                        cpu->reorderBuffer->array[i].loadStoreCounter = stage->loadStoreCounter;
                        break;
                    }
                }
                stage->stalled=0;
...
//        printf("Why isn't this working - %s %d %d\n",stage->opcode,cpu->physicalRegisters[stage->rd].isValid,stage->rd);
        ...
        if(strcmp(stage->opcode,"")==0){
            ...
            //checkForBackToBackExecution(cpu); // MARK: - remove test
        ...
        else if(isInstructionLoad(stage->opcode) || isInstructionLDR(stage->opcode)){
...
//                printf("Commit load -> This should be the third cycle%d \n",stage->loadStoreCounter);
        ...
        } else if(isInstructionStore(stage->opcode) || isInstructionSTR(stage->opcode)){
            //stage->index ==
            //&& (stage->index == peekLSQHead(cpu->loadStoreQueue).index
//            printf("Commit Store -** load store count - %d\n",cpu->stage[MEM].loadStoreCounter);
//            printf("Commit Store - load store count - %d\n",stage->loadStoreCounter);
//    ----        stage->loadStoreCounter =stage->loadStoreCounter -1;
            if(stage->loadStoreCounter == 0) { // MARK: - change store commit
...
void printContentsOfArchitectureRegister(APEX_CPU* cpu){
    ...
    for(int i=0;i<48;i++){
        printf("|[%d]->%-9d |\t",i,cpu->architectureRegisters[i]);
        if(i%5==4){
            printf("\n");
        }
    }
...
void printBTBList(APEX_CPU* cpu){
